...
internal class TPasswordReader
{
    private readonly IDbContextFactory<TableDbContext> _context;

    internal TPasswordReader(IDbContextFactory<TableDbContext> context)
    {
        _context = context;
    }

    internal async Task<bool> AssertAsync(Guid password, CancellationToken token)
    {
    ...
    internal async Task<TPasswordEntity?> FetchAsync(Guid password, CancellationToken token)
    {
        using var db = _context.CreateDbContext();
    ...
    internal async Task<int> CountAsync(IPasswordCriteria criteria, CancellationToken token)
    {
        return await BuildQuery(criteria)
            .CountAsync(token);
    }

    internal async Task<IEnumerable<TPasswordEntity>> CollectAsync(IPasswordCriteria criteria, CancellationToken token)
    {
        return await BuildQuery(criteria)
