...
public class PartitionController : ControllerBase
{
    ...
    [HttpGet(Endpoints.SecurityApi.Identification.Partition.Assert)]
    ...
    public async Task<ActionResult<int>> CountAsync([FromQuery] IPartitionCriteria criteria, CancellationToken token)
    {
        var count = await _partitionService.CountAsync(criteria, token);
    ...
    public async Task<ActionResult<IEnumerable<PartitionModel>>> CollectAsync([FromQuery] IPartitionCriteria criteria, CancellationToken token)
    {
        var models = await _partitionService.CollectAsync(criteria, token);
    ...
    public async Task<ActionResult<IEnumerable<PartitionMatch>>> SearchAsync([FromQuery] IPartitionCriteria criteria, CancellationToken token)
    {
        var matches = await _partitionService.SearchAsync(criteria, token);
