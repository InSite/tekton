...
[ApiController]
[ApiExplorerSettings(GroupName = "Content: Text")]
[Authorize]
public class TranslationController : ControllerBase
{
    ...
    [HttpGet(Endpoints.ContentApi.Text.Translation.Assert)]
    ...
    public async Task<ActionResult<int>> CountAsync([FromQuery] ITranslationCriteria criteria, CancellationToken token)
    {
        var count = await _translationService.CountAsync(criteria, token);
    ...
    public async Task<ActionResult<IEnumerable<TranslationModel>>> CollectAsync([FromQuery] ITranslationCriteria criteria, CancellationToken token)
    {
        var models = await _translationService.CollectAsync(criteria, token);
    ...
    public async Task<ActionResult<IEnumerable<TranslationMatch>>> SearchAsync([FromQuery] ITranslationCriteria criteria, CancellationToken token)
    {
        var matches = await _translationService.SearchAsync(criteria, token);
